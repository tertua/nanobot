...
class WebSocketChannel(BaseChannel):
    ...
    def __init__(
        ...
        *,
        ...
        self._temporary_attachments: dict[str, list[str]] = {}
    ...
    def _claim_temporary_chat(
        ...
        if (
            ...
            or sessions is None
            or self.gateway.cancel_active_turn is None
        ):
            return "temporary_chat_unavailable"
        subscribers = self._subs.get(chat_id, set())
        if subscribers and connection not in subscribers:
            return "temporary_chat_not_owned"
        current = next(
            (
                candidate
                for candidate in self._conn_chats.get(connection, ())
                if _is_temporary_chat_id(candidate)
            ),
            None,
        )
        if current is not None and current != chat_id:
            return "temporary_chat_in_use"
        sessions.get_or_create_transient(f"websocket:{chat_id}")
    ...
    async def _discard_temporary_chat(
        ...
        sessions = self.gateway.session_manager
        cancel = self.gateway.cancel_active_turn
        assert sessions is not None and cancel is not None
        sessions.discard_transient(session_key)
        try:
            await cancel(session_key)
        finally:
            clear_websocket_turns(chat_id)
            self._clear_stream_buffers(chat_id)
            self._media.discard_inbound_attachments(
                self._temporary_attachments.pop(chat_id, []),
            )
        return None
    ...
    async def _cleanup_connection(self, connection: ServerConnection) -> None:
        ...
        try:
            temporary_chat_id = next(
                (
                    chat_id
                    for chat_id in self._conn_chats.get(connection, ())
        ...
        if t == "message":
            ...
            if raw_media is not None:
                ...
                if reason is not None:
                    ...
                    return
                if temporary:
                    self._temporary_attachments.setdefault(cid, []).extend(media_paths)
